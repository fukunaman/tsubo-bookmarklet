// ==UserScript==
// @name         坪単価バッジ（Athome & ふれんず）
// @namespace    https://github.com/s
// @version      1.0.0
// @description  Athome/ふれんずで坪単価バッジを自動表示します
...
(function () {
  ...
  const injectBadges = () => {
    ...
    try {
      ...
      console.error('[tsubo-badge] injection failed', error);
    } finally {
      isRunning = false;
    }
  };
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', injectBadges, { once: true });
  } else {
    injectBadges();
  }
  ...
  window.addEventListener('pageshow', injectBadges);
})();
